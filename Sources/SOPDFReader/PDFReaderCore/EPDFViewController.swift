...
class EPDFViewController: UIViewController, UIPageViewControllerDelegate, UIPageViewControllerDataSource, UIScrollViewDelegate {
    ...
    // MARK: - Initializer
    convenience init?(fileURL: URL, password: String? = nil, hasCoverPage: Bool = false, layoutMargin: EPDFLayoutMargin = EPDFLayoutMargin.zero, backgroundColor: UIColor = UIColor.lightGray, pageBackgroundColor: UIColor = UIColor.white) {
        if let pdfDocumentController = EPDFDocumentController(fileURL: fileURL, password: password, pageBackgroundColor: pageBackgroundColor) {
    ...
    override func viewDidLoad() {
        ...
        var viewControllers = [EPDFPageViewController]()
        ...
        if isLandscape && self.pdfDocumentController.numberOfPages != 1 {
        ...
        self.pageViewController.gestureRecognizers.forEach {
            zoomInGestureRecognizer.require(toFail: $0)
        }
        self.pageViewController.gestureRecognizers.forEach {
            navigationBarGestureRecognizer.require(toFail: $0)
        }
...
//        self.pageViewController.setViewControllers([controller], direction: .forward, animated: true, completion: nil)
    ...
    internal func pageViewController(_ pageViewController: UIPageViewController, spineLocationFor orientation: UIInterfaceOrientation) -> UIPageViewController.SpineLocation {
        ...
        let pageNumber = (self.pageViewController.viewControllers?.first as? EPDFPageViewController)?.pageNumber ?? 1
