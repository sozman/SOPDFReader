...
class EPDFDocumentController {
    ...
    private var viewControllerCache = [Int: EPDFPageViewController]()
    ...
    init?(fileURL: URL, password: String?, pageBackgroundColor: UIColor) {
        ...
        if let document = self.document {
            ...
            if numberOfPages > 0 {
                ...
                NotificationCenter.default.addObserver(self, selector: #selector(handleMemoryWarning), name: UIApplication.didBecomeActiveNotification, object: nil)
                return
            }
        }
    ...
    func pageViewController(at pageNumber: Int) -> EPDFPageViewController? {
        // Manage Cache
        ...
        var newViewControllerCache = [Int: EPDFPageViewController]()
        ...
        let pageNumbers = Array(minCachePageNumberToCreate...maxCachePageNumberToCreate).sorted {
            abs($0 - pageNumber) < abs($1 - pageNumber)
        }
        for pageNumber in pageNumbers {
            if newViewControllerCache[pageNumber] == nil {
                newViewControllerCache[pageNumber] = EPDFPageViewController(page: self.document!.page(at: pageNumber), box: self.box, backgroundColor: self.pageBackgroundColor)
            }
        }
    ...
    var emptyPageViewController: EPDFPageViewController {
        return EPDFPageViewController(page: nil, box: self.box, backgroundColor: UIColor.clear)
    }
